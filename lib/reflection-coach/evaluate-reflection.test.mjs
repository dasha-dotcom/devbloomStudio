...
test("all-about-me recognizes partial and complete reflections", () => {
  ...
  const personalizedAddedTitleReflection = getEvaluation(
    "all-about-me",
    'One thing I changed in the HTML was the text. It changed my page by adding the big title "cats" to it.',
  );
  assert.equal(personalizedAddedTitleReflection.coachResult, "strong");
  assert.equal(personalizedAddedTitleReflection.detectedSignals.hasCopiedExample, false);
  assert.equal(personalizedAddedTitleReflection.followUpQuestion, undefined);
...
test("copied project examples ask students to personalize", () => {
  ...
  const vaguePromptCopies = [
    ...
    [
      "all-about-me",
      "One thing I changed in the HTML was the text. It changed my page by adding text",
    ],
    [
      "vibe-page",
